...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for(int i=1; i<=100;i++){
            //i값이 1부터 100부터 될 때까지 100개의 타일이 만들어짐
            Tile tile = new Tile(i);
            tiles.add(tile);
        }
        //타일 100개를 집어넣고 setDesc를 통해 설명을 집어넣음
        //highway
        ...
        //사용자의 지금 현재 위치를 알아야 한다. -> 데이터 타입 : Tile (Tile 안에 int 존재)
        int current =0;
        ...
        while(true){
            ...
            System.out.println("쭈사위 눈은 :" +value);



            //어떤 타일을 가져와야 하나? Current + vaule 의 타일을 가져와야 한다.
            //Current는 Current + Value. 로직부터 While루프(몇번반복해야할지 모르기때문에)

            current = current+value;
            System.out.println("현재위치 : " +current);
